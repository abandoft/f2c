...
static F2cExpr *parse_postfix(AstParser *parser, const F2cToken *name_token) {
    ...
    if (kind == F2C_EXPR_STRUCTURE_CONSTRUCTOR) {
        ...
        if (strcmp(expression->text, "real") == 0 && expression->child_count >= 2U) {
            ...
            Type kind_type = f2c_ast_kind_type_from_argument(expression->children[2]);
        ...
        if (expression->rank != 0U) {
            for (i = 0U; i < expression->child_count; ++i) {
                if (expression->children[i]->rank == expression->rank) {
                    f2c_ast_copy_expression_shape(expression, &expression->children[i]->shape);
                    break;
                }
            }
        }
        ...
        if (expression->child_count != 0U) {
            const F2cExpr *first_argument =
                f2c_ast_intrinsic_argument_value(expression->children[0]);
