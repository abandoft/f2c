...
static void emit_exponential_renderer(Buffer *output) {
    f2c_buffer_append(
        ...
        "size_t total; char exponent_buffer[32]; char marker = descriptor->code[0] == 'D' ? 'D' "
        ": 'E'; if (!isfinite(value)) { int needed = snprintf(NULL, 0U, state->sign_plus ? "
        ...
        "pow(10.0, (double)exponent); } fraction = descriptor->digits; if (scale > 1) fraction "
        ...
        "descriptor->exponent)) { free(mantissa); state->status = 0; return NULL; } total = "
        "mantissa_length + (size_t)exponent_length + 2U; if (total < mantissa_length) { "
        ...
        "memcpy(field, mantissa, mantissa_length); field[mantissa_length] = marker; "
