...
void f2c_parse_contiguous_declaration(Context *context, Unit *unit, Line *source_line) {
    size_t index = source_line != NULL && source_line->token_count > 1U &&
                       ...
                       ? 1U
                       : 0U;
    if (!f2c_line_token_equals(source_line, index, "contiguous"))
        return;
    ++index;
    if (index < source_line->token_count &&
        ...
        ++index;
    if (index == source_line->token_count) {
        f2c_diagnostic(context, source_line->number, 1, "CONTIGUOUS declaration has no entities");
        return;
    }
    while (index < source_line->token_count) {
        ...
        if (source_line->tokens[index].kind != F2C_TOKEN_IDENTIFIER) {
            f2c_diagnostic_token_code(context, F2C_DIAGNOSTIC_SYNTAX, source_line,
                                      ...
                                      "malformed CONTIGUOUS declaration entity");
            return;
        }
        ...
        if (symbol == NULL) {
            f2c_diagnostic_code(context, F2C_DIAGNOSTIC_OUT_OF_MEMORY, source_line->number, 1,
                                "out of memory in CONTIGUOUS declaration");
            free(name);
            return;
        }
        symbol->contiguous = 1;
        ...
        if (source_line->tokens[index].kind != F2C_TOKEN_COMMA ||
            ...
            f2c_diagnostic(context, source_line->number, 1,
                           "malformed CONTIGUOUS declaration entity list");
            return;
        }
    }
}
