...
F2cResult f2c_transpile_project_config(const F2cInput *inputs, size_t input_count,
    ...
    if (context.result.error_count == 0U) {
        ...
        f2c_buffer_append(
            ...
            "(ptrdiff_t)step); }\n"
            "static inline F2C_UNUSED size_t f2c_section_extent(int64_t first, int64_t last, "
            "int64_t step) { uint64_t distance; uint64_t magnitude; uint64_t quotient; if "
            "(step == 0) abort(); if (step > 0) { if (last < first) return 0U; distance = "
            "(uint64_t)last - (uint64_t)first; magnitude = (uint64_t)step; } else { if (last > "
            "first) return 0U; distance = (uint64_t)first - (uint64_t)last; magnitude = "
            "(uint64_t)(-(step + 1)) + UINT64_C(1); } quotient = distance / magnitude; if "
            "(quotient >= (uint64_t)SIZE_MAX) abort(); return (size_t)(quotient + "
            "UINT64_C(1)); }\n"
            "static inline F2C_UNUSED bool f2c_descriptor_is_contiguous(size_t rank, const "
            "size_t *extents, const ptrdiff_t *strides) { size_t dimension; ptrdiff_t expected = "
            "1; for (dimension = 0U; dimension < rank; ++dimension) if (extents[dimension] == 0U) "
            "return true; for (dimension = 0U; dimension < rank; ++dimension) { if "
            "(strides[dimension] != expected) return false; expected = "
            "f2c_descriptor_stride_extent(expected, extents[dimension]); } return true; }\n");
        f2c_buffer_append(
            &context.output,
            ...
            "static inline F2C_UNUSED size_t f2c_descriptor_element_count(const "
            "f2c_descriptor *descriptor) { size_t dimension; size_t count = 1U; if "
            "(descriptor == NULL || descriptor->rank == 0U || descriptor->rank > 15U) abort(); "
            "for (dimension = 0U; dimension < descriptor->rank; ++dimension) { size_t extent; "
            "if (descriptor->extent[dimension] < 0) abort(); extent = "
            "(size_t)descriptor->extent[dimension]; if (extent == 0U) return 0U; if (count > "
            "SIZE_MAX / extent) abort(); count *= extent; } return count; }\n"
            "static inline F2C_UNUSED size_t f2c_descriptor_record_size(const f2c_descriptor "
            "*descriptor, bool character) { if (descriptor == NULL) abort(); return character "
        ...
        f2c_buffer_append(
            &context.output,
            "static inline F2C_UNUSED bool f2c_associated_array_target(const void *pointer_data, "
            "size_t pointer_rank, const size_t *pointer_extents, const ptrdiff_t "
            "*pointer_strides, const void *target_data, size_t element_size, size_t source_rank, "
            "const int64_t *source_lowers, const size_t *source_extents, const ptrdiff_t "
            "*source_strides, const bool *sections, const int64_t *firsts, const int64_t *lasts, "
            "const int64_t *steps) { size_t pointer_dimension; size_t source_dimension; size_t "
            "result_dimension = 0U; ptrdiff_t offset = 0; ptrdiff_t byte_offset; if "
            "(pointer_data == NULL || target_data == NULL) return false; for (pointer_dimension = "
            "0U; pointer_dimension < pointer_rank; ++pointer_dimension) if "
            "(pointer_extents[pointer_dimension] == 0U) return false; for (source_dimension = 0U; "
            "source_dimension < source_rank; "
            "++source_dimension) { int64_t first = firsts[source_dimension]; size_t ordinal = "
            "f2c_array_offset(first, source_lowers[source_dimension], "
            "source_extents[source_dimension]); ptrdiff_t contribution; if (ordinal > "
            "(size_t)PTRDIFF_MAX) abort(); contribution = f2c_descriptor_stride_multiply("
            "(ptrdiff_t)ordinal, source_strides[source_dimension]); offset = "
            "f2c_descriptor_offset_add(offset, contribution); if (sections[source_dimension]) { "
            "size_t extent; ptrdiff_t stride; if (result_dimension >= pointer_rank) return false; "
            "extent = f2c_section_extent(first, lasts[source_dimension], "
            "steps[source_dimension]); stride = f2c_descriptor_stride_step("
            "source_strides[source_dimension], steps[source_dimension]); if (pointer_extents["
            "result_dimension] != extent || pointer_strides[result_dimension] != stride) return "
            "false; ++result_dimension; } } if (result_dimension != pointer_rank || element_size "
            "> (size_t)PTRDIFF_MAX) return false; byte_offset = "
            "f2c_descriptor_stride_multiply(offset, (ptrdiff_t)element_size); return (const "
            "unsigned char *)pointer_data == (const unsigned char *)target_data + byte_offset; "
            "}\n");
        f2c_buffer_append(
            &context.output,
