    ...
    if (context.result.error_count == 0U) {
        ...
        if (needs_complex) {
            ...
            f2c_buffer_append(
                ...
                "f2c_complex_float b) { float ar = crealf(a), ai = cimagf(a), br = crealf(b), "
                "bi = cimagf(b), logbw = logbf(fmaxf(fabsf(br), fabsf(bi))), real_part, "
                "imag_part; int exponent = 0; if (isfinite(logbw)) { exponent = (int)logbw; br "
                "= scalbnf(br, -exponent); bi = scalbnf(bi, -exponent); } { float denominator = "
                "br * br + bi * bi; real_part = scalbnf((ar * br + ai * bi) / denominator, "
                "-exponent); imag_part = scalbnf((ai * br - ar * bi) / denominator, -exponent); "
                "} if (isnan(real_part) && isnan(imag_part)) { if (br == 0.0f && bi == 0.0f && "
                "(!isnan(ar) || !isnan(ai))) { real_part = copysignf(INFINITY, br) * ar; "
                "imag_part = copysignf(INFINITY, br) * ai; } else if ((isinf(ar) || isinf(ai)) "
                "&& isfinite(br) && isfinite(bi)) { ar = copysignf(isinf(ar) ? 1.0f : 0.0f, "
                "ar); ai = copysignf(isinf(ai) ? 1.0f : 0.0f, ai); real_part = INFINITY * (ar "
                "* br + ai * bi); imag_part = INFINITY * (ai * br - ar * bi); } else if "
                "(isinf(logbw) && logbw > 0.0f && isfinite(ar) && isfinite(ai)) { br = "
                "copysignf(isinf(br) ? 1.0f : 0.0f, br); bi = copysignf(isinf(bi) ? 1.0f : "
                "0.0f, bi); real_part = 0.0f * (ar * br + ai * bi); imag_part = 0.0f * (ai * br "
                "- ar * bi); } } return f2c_make_c(real_part, imag_part); }\n"
                "static inline F2C_UNUSED f2c_complex_double f2c_zdiv(f2c_complex_double a, "
                "f2c_complex_double b) { double ar = creal(a), ai = cimag(a), br = creal(b), "
                "bi = cimag(b), logbw = logb(fmax(fabs(br), fabs(bi))), real_part, imag_part; int "
                "exponent = 0; if (isfinite(logbw)) { exponent = (int)logbw; br = scalbn(br, "
                "-exponent); bi = scalbn(bi, -exponent); } { double denominator = br * br + bi "
                "* bi; real_part = scalbn((ar * br + ai * bi) / denominator, -exponent); "
                "imag_part = scalbn((ai * br - ar * bi) / denominator, -exponent); } if "
                "(isnan(real_part) && isnan(imag_part)) { if (br == 0.0 && bi == 0.0 && "
                "(!isnan(ar) || !isnan(ai))) { real_part = copysign(INFINITY, br) * ar; "
                "imag_part = copysign(INFINITY, br) * ai; } else if ((isinf(ar) || isinf(ai)) "
                "&& isfinite(br) && isfinite(bi)) { ar = copysign(isinf(ar) ? 1.0 : 0.0, ar); "
                "ai = copysign(isinf(ai) ? 1.0 : 0.0, ai); real_part = INFINITY * (ar * br + ai "
                "* bi); imag_part = INFINITY * (ai * br - ar * bi); } else if (isinf(logbw) && "
                "logbw > 0.0 && isfinite(ar) && isfinite(ai)) { br = copysign(isinf(br) ? 1.0 "
                ": 0.0, br); bi = copysign(isinf(bi) ? 1.0 : 0.0, bi); real_part = 0.0 * (ar * "
                "br + ai * bi); imag_part = 0.0 * (ai * br - ar * bi); } } return "
                "f2c_make_z(real_part, imag_part); }\n");
        }
        if (needs_max) {
            f2c_buffer_append(
                &context.output,
        ...
        if (needs_io) {
            ...
            if (needs_complex) {
                f2c_buffer_append(
                    &context.output,
                    "static inline F2C_UNUSED void f2c_write_c(FILE *f, f2c_complex_float v) { "
                    "fprintf(f, \" "
                    "(%.9g,%.9g)\", (double)crealf(v), (double)cimagf(v)); }\n"
                    "static inline F2C_UNUSED void f2c_write_z(FILE *f, f2c_complex_double v) { "
                    "fprintf(f, \" "
                    ...
                    "static inline F2C_UNUSED int f2c_read_c(FILE *f, f2c_complex_float *v) { "
                    "double "
                    "r, i; int n = f2c_read_complex_parts(f, &r, &i); if (n == 2) *v = "
                    "f2c_make_c((float)r, (float)i); return n; }\n"
                    "static inline F2C_UNUSED int f2c_read_z(FILE *f, f2c_complex_double *v) { "
                    "double "
                    "r, i; int n = f2c_read_complex_parts(f, &r, &i); if (n == 2) *v = "
            ...
            f2c_buffer_append(
                ...
                needs_complex
                    ? "#define F2C_WRITE(f, v) _Generic((v), int8_t: f2c_write_i8, int16_t: "
                      ...
                      "f2c_write_string, f2c_complex_float: f2c_write_c, f2c_complex_double: "
                      "f2c_write_z)((f), (v))\n"
