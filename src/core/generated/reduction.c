...
void f2c_emit_reduction_support(Buffer *output, int needs_complex) {
    ...
    f2c_buffer_append(
        ...
        "F2C_REDUCTION_Z };\n"
        "static inline F2C_UNUSED int64_t f2c_reduction_integer_result(int64_t value, int kind) "
        "{ int64_t low = kind == 1 ? INT8_MIN : kind == 2 ? INT16_MIN : kind == 4 ? "
        "INT32_MIN : kind == 8 ? INT64_MIN : INT64_C(1); int64_t high = kind == 1 ? "
        "INT8_MAX : kind == 2 ? INT16_MAX : kind == 4 ? INT32_MAX : kind == 8 ? "
        "INT64_MAX : INT64_C(0); if (value < low || value > high) abort(); return value; }\n"
    ...
    f2c_buffer_append(
        ...
        "static inline F2C_UNUSED bool f2c_reduction_selected(const void *mask, size_t size, "
        "ptrdiff_t stride, bool scalar, size_t index) { return scalar && (mask == NULL || "
        "f2c_reduction_logical_at(mask, size, (ptrdiff_t)index * stride)); }\n");
    f2c_buffer_append(
        output,
        "#define F2C_DEFINE_MASKED_REDUCTIONS(s, t, zero, one, low, high) "
        "static inline F2C_UNUSED t f2c_sum_mask_##s(const t *v, size_t n, ptrdiff_t d, "
        "const void *m, size_t ms, ptrdiff_t md, bool scalar) { size_t i; t r = (zero); "
        "for (i = 0U; i < n; ++i) if (f2c_reduction_selected(m, ms, md, scalar, i)) "
        "r = (t)(r + v[(ptrdiff_t)i * d]); return r; } "
        "static inline F2C_UNUSED t f2c_product_mask_##s(const t *v, size_t n, ptrdiff_t d, "
        "const void *m, size_t ms, ptrdiff_t md, bool scalar) { size_t i; t r = (one); "
        "for (i = 0U; i < n; ++i) if (f2c_reduction_selected(m, ms, md, scalar, i)) "
        "r = (t)(r * v[(ptrdiff_t)i * d]); return r; } "
        "static inline F2C_UNUSED t f2c_maxval_mask_##s(const t *v, size_t n, ptrdiff_t d, "
        "const void *m, size_t ms, ptrdiff_t md, bool scalar) { size_t i; t r = (low); "
        "for (i = 0U; i < n; ++i) if (f2c_reduction_selected(m, ms, md, scalar, i) && "
        "v[(ptrdiff_t)i * d] > r) r = v[(ptrdiff_t)i * d]; return r; } "
        "static inline F2C_UNUSED t f2c_minval_mask_##s(const t *v, size_t n, ptrdiff_t d, "
        "const void *m, size_t ms, ptrdiff_t md, bool scalar) { size_t i; t r = (high); "
        "for (i = 0U; i < n; ++i) if (f2c_reduction_selected(m, ms, md, scalar, i) && "
        "v[(ptrdiff_t)i * d] < r) r = v[(ptrdiff_t)i * d]; return r; } "
        "static inline F2C_UNUSED int64_t f2c_maxloc_mask_##s(const t *v, size_t n, "
        "ptrdiff_t d, const void *m, size_t ms, ptrdiff_t md, bool scalar, bool back) { "
        "size_t i, p = 0U; bool found = false; for (i = 0U; i < n; ++i) if "
        "(f2c_reduction_selected(m, ms, md, scalar, i) && (!found || "
        "v[(ptrdiff_t)i * d] > v[(ptrdiff_t)p * d] || "
        "(back && v[(ptrdiff_t)i * d] == v[(ptrdiff_t)p * d]))) { p = i; found = true; } "
        "return found && p < (size_t)INT64_MAX ? (int64_t)p + INT64_C(1) : "
        "(found ? (abort(), INT64_C(0)) : INT64_C(0)); } "
        "static inline F2C_UNUSED int64_t f2c_minloc_mask_##s(const t *v, size_t n, "
        "ptrdiff_t d, const void *m, size_t ms, ptrdiff_t md, bool scalar, bool back) { "
        "size_t i, p = 0U; bool found = false; for (i = 0U; i < n; ++i) if "
        "(f2c_reduction_selected(m, ms, md, scalar, i) && (!found || "
        "v[(ptrdiff_t)i * d] < v[(ptrdiff_t)p * d] || "
        ...
        "return found && p < (size_t)INT64_MAX ? (int64_t)p + INT64_C(1) : "
        "(found ? (abort(), INT64_C(0)) : INT64_C(0)); }\n"
        "F2C_DEFINE_MASKED_REDUCTIONS(i8, int8_t, INT8_C(0), INT8_C(1), INT8_MIN, INT8_MAX)\n"
        "F2C_DEFINE_MASKED_REDUCTIONS(i16, int16_t, INT16_C(0), INT16_C(1), INT16_MIN, "
        "INT16_MAX)\n"
        "F2C_DEFINE_MASKED_REDUCTIONS(i32, int32_t, INT32_C(0), INT32_C(1), INT32_MIN, "
        "INT32_MAX)\n"
        "F2C_DEFINE_MASKED_REDUCTIONS(i64, int64_t, INT64_C(0), INT64_C(1), INT64_MIN, "
        "INT64_MAX)\n"
        "F2C_DEFINE_MASKED_REDUCTIONS(f, float, 0.0f, 1.0f, -HUGE_VALF, HUGE_VALF)\n"
        "F2C_DEFINE_MASKED_REDUCTIONS(d, double, 0.0, 1.0, -HUGE_VAL, HUGE_VAL)\n"
        "#undef F2C_DEFINE_MASKED_REDUCTIONS\n");
    if (needs_complex) {
        f2c_buffer_append(
            output,
            "static inline F2C_UNUSED f2c_complex_float f2c_sum_mask_c("
            "const f2c_complex_float *v, size_t n, ptrdiff_t d, const void *m, size_t ms, "
            "ptrdiff_t md, bool scalar) { size_t i; f2c_complex_float r = "
            "f2c_make_c(0.0f, 0.0f); for (i = 0U; i < n; ++i) if "
            "(f2c_reduction_selected(m, ms, md, scalar, i)) "
            "r = f2c_cadd(r, v[(ptrdiff_t)i * d]); return r; }\n"
            "static inline F2C_UNUSED f2c_complex_double f2c_sum_mask_z("
            "const f2c_complex_double *v, size_t n, ptrdiff_t d, const void *m, size_t ms, "
            "ptrdiff_t md, bool scalar) { size_t i; f2c_complex_double r = "
            "f2c_make_z(0.0, 0.0); for (i = 0U; i < n; ++i) if "
            "(f2c_reduction_selected(m, ms, md, scalar, i)) "
            "r = f2c_zadd(r, v[(ptrdiff_t)i * d]); return r; }\n"
            "static inline F2C_UNUSED f2c_complex_float f2c_product_mask_c("
            "const f2c_complex_float *v, size_t n, ptrdiff_t d, const void *m, size_t ms, "
            "ptrdiff_t md, bool scalar) { size_t i; f2c_complex_float r = "
            "f2c_make_c(1.0f, 0.0f); for (i = 0U; i < n; ++i) if "
            "(f2c_reduction_selected(m, ms, md, scalar, i)) "
            "r = f2c_cmul(r, v[(ptrdiff_t)i * d]); return r; }\n"
            "static inline F2C_UNUSED f2c_complex_double f2c_product_mask_z("
            "const f2c_complex_double *v, size_t n, ptrdiff_t d, const void *m, size_t ms, "
            "ptrdiff_t md, bool scalar) { size_t i; f2c_complex_double r = "
            "f2c_make_z(1.0, 0.0); for (i = 0U; i < n; ++i) if "
            "(f2c_reduction_selected(m, ms, md, scalar, i)) "
            "r = f2c_zmul(r, v[(ptrdiff_t)i * d]); return r; }\n");
    }
    f2c_buffer_append(
    ...
    f2c_buffer_append(
        output,
        needs_complex
            ? "#define F2C_SUM_MASK(v, n, d, m, ms, md, s) _Generic(*(v), int8_t: "
              "f2c_sum_mask_i8, int16_t: f2c_sum_mask_i16, int32_t: f2c_sum_mask_i32, "
              "int64_t: f2c_sum_mask_i64, float: f2c_sum_mask_f, double: f2c_sum_mask_d, "
              "f2c_complex_float: f2c_sum_mask_c, f2c_complex_double: "
              "f2c_sum_mask_z)((v), (n), (d), (m), (ms), (md), (s))\n"
              "#define F2C_PRODUCT_MASK(v, n, d, m, ms, md, s) _Generic(*(v), int8_t: "
              "f2c_product_mask_i8, int16_t: f2c_product_mask_i16, int32_t: "
              "f2c_product_mask_i32, int64_t: f2c_product_mask_i64, float: "
              "f2c_product_mask_f, double: f2c_product_mask_d, f2c_complex_float: "
              "f2c_product_mask_c, f2c_complex_double: f2c_product_mask_z)"
              "((v), (n), (d), (m), (ms), (md), (s))\n"
            : "#define F2C_SUM_MASK(v, n, d, m, ms, md, s) _Generic(*(v), int8_t: "
              "f2c_sum_mask_i8, int16_t: f2c_sum_mask_i16, int32_t: f2c_sum_mask_i32, "
              "int64_t: f2c_sum_mask_i64, float: f2c_sum_mask_f, double: "
              "f2c_sum_mask_d)((v), (n), (d), (m), (ms), (md), (s))\n"
              "#define F2C_PRODUCT_MASK(v, n, d, m, ms, md, s) _Generic(*(v), int8_t: "
              "f2c_product_mask_i8, int16_t: f2c_product_mask_i16, int32_t: "
              "f2c_product_mask_i32, int64_t: f2c_product_mask_i64, float: "
              "f2c_product_mask_f, double: f2c_product_mask_d)"
              "((v), (n), (d), (m), (ms), (md), (s))\n");
    f2c_buffer_append(
        output,
        ...
        "#define F2C_MAXIMUM_MASK(v, n, d, m, ms, md, s) _Generic(*(v), int8_t: "
        "f2c_maxval_mask_i8, int16_t: f2c_maxval_mask_i16, int32_t: f2c_maxval_mask_i32, "
        "int64_t: f2c_maxval_mask_i64, float: f2c_maxval_mask_f, double: "
        "f2c_maxval_mask_d)((v), (n), (d), (m), (ms), (md), (s))\n"
        "#define F2C_MINIMUM_MASK(v, n, d, m, ms, md, s) _Generic(*(v), int8_t: "
        "f2c_minval_mask_i8, int16_t: f2c_minval_mask_i16, int32_t: f2c_minval_mask_i32, "
        "int64_t: f2c_minval_mask_i64, float: f2c_minval_mask_f, double: "
        "f2c_minval_mask_d)((v), (n), (d), (m), (ms), (md), (s))\n"
        "#define F2C_MAXIMUM_LOCATION_MASK(v, n, d, m, ms, md, s, b) _Generic(*(v), "
        "int8_t: f2c_maxloc_mask_i8, int16_t: f2c_maxloc_mask_i16, int32_t: "
        "f2c_maxloc_mask_i32, int64_t: f2c_maxloc_mask_i64, float: f2c_maxloc_mask_f, "
        "double: f2c_maxloc_mask_d)((v), (n), (d), (m), (ms), (md), (s), (b))\n"
        "#define F2C_MINIMUM_LOCATION_MASK(v, n, d, m, ms, md, s, b) _Generic(*(v), "
        "int8_t: f2c_minloc_mask_i8, int16_t: f2c_minloc_mask_i16, int32_t: "
        "f2c_minloc_mask_i32, int64_t: f2c_minloc_mask_i64, float: f2c_minloc_mask_f, "
        "double: f2c_minloc_mask_d)((v), (n), (d), (m), (ms), (md), (s), (b))\n");
