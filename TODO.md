...
- [ ] 完成模块 `PUBLIC/PRIVATE`、`ONLY`、重命名、泛型接口、运算符/赋值泛型和模块过程。普通
  ...
  硬编码特例。命名泛型现在保存完整的类型化候选集；显式接口体和模块内
  `MODULE PROCEDURE [::] specific-list` 均会建立具体过程签名，按实参 type/kind/rank、关键字和
  过程类别选择唯一候选，并在无匹配、歧义、重复或未定义具体过程时于精确 token 处硬失败。
  候选集支持 `USE ONLY`、重命名、直接与包装模块的同源合并及默认私有模块中的显式公开，生成端
  始终调用真正的提供者具体过程；结构化语法 AST、动态 128 项边界、严格生成 C17、ASan/UBSan
  以及模块过程和外部显式接口两类 gfortran 差分均已覆盖。运算符/赋值泛型的实际绑定、接口中的
  `PROCEDURE` 具体过程列表、子模块及 `LA_CONSTANTS` 通用化仍未实现，因此本项保持未关闭。
