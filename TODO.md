...
- [ ] 补齐 `PROGRAM`、`MODULE`、`BLOCK DATA`、`SUBROUTINE`、`FUNCTION`、`ENTRY`、内部过程
  ...
  规格表达式，严格 C17、ASan/UBSan 与原生 gfortran
  差分已经进入 CI。当前对宿主局部可分配/指针描述符、宿主过程指针，以及把带捕获的内部过程
  作为普通过程值逃逸的用法给出生成前硬错误；任意层嵌套、闭包感知过程值及完整派生/多态动态
  实体捕获仍未实现。宿主已有显式接口的外部过程会继承完整签名，但跨作用域的接口兼容组合仍需
  继续补齐。
  用户定义
