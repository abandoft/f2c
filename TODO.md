...
- [ ] 旧式标号 `DO` 的结构改写目前只能把合成 token 回指到原语句起点，尚未提供逐片段精确映射；
  预处理宏展开也没有 expansion/spelling 双位置。还需让所有 parser/semantic 诊断直接消费 token/
  AST span，并增加 related location，才能满足“任意诊断均有精确范围”的验收标准。
...
- [ ] 将数值清单中的套件数、记录数、配对数和摘要作为单一机器可读来源，README/TODO/CI 不得
  手工复制出互相矛盾的统计值。当前 `README.md` 与 `README-ZH.md` 仍保留旧的 5,807,941 条
  统计，需由清单自动生成并纠正。
...
- [ ] CLI 已支持 stdin/stdout，输入使用有预算的流式增长读取，不再依赖 `fseek/ftell`；C 与头文件
  会全部暂存成功后再事务提交，提交失败恢复原文件。仍需响应文件或项目清单、include/macro 选项、
  机器可读诊断及跨进程崩溃恢复日志。
- [ ] 为嵌入式使用提供不依赖 seek、文件系统或进程全局状态的流式输入/输出接口。

### P1-TEST-01 测试体系

- [ ] 按 lexer、parser、AST、语义、常量折叠、intrinsic、I/O、生命周期、IR 和 emitter 建立独立
  单元测试，并保留项目级端到端测试。
...
- [x] 请求级预算覆盖输入字节、逻辑行、token、AST 节点、表达式树/解析深度、常量求值步数、
  诊断及生成物；循环参数引用、超深表达式和各类超限路径均可预测失败且不返回部分输出。
