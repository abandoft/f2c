...
- [x] 当前本地严格 AppleClang 静态 Debug、静态 Release、共享 Release 与 ASan/UBSan Debug
  构建基线已经建立；本轮普通与 ASan/UBSan 严格 CTest 均为 36/36，架构边界检查作为独立测试运行。
...
- [ ] 完成模块 `PUBLIC/PRIVATE`、`ONLY`、重命名、泛型接口、运算符/赋值泛型和模块过程。普通
  ...
  再把其 `USE`、隐式规则或派生类型泄漏到宿主模块。`LA_CONSTANTS` 已支持完整导入和重命名，
  但仍是硬编码特例；模块默认/显式可见性、泛型接口及运算符/赋值设计符的实际绑定、完整模块过程
  仍未实现，因此本项保持未关闭。
