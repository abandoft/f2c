...
- [x] 当前本地严格 AppleClang 静态 Debug、静态 Release、共享 Release 与 ASan/UBSan Debug
  构建基线已经建立；默认严格 CTest 必须全部通过，架构边界检查作为独立测试运行。
...
- [ ] 完成标量、数组、数组段、向量下标、数组构造器、隐式 DO、子串、组件引用和函数结果的
  ...
  无 `PAD/ORDER` 的 `RESHAPE` 视图；运行时会验证动态合形。普通数组段赋值也会用统一 typed
  extent 规则一次求出目标和值的逐维 shape，在分配快照和首次写入前硬校验合形；空段、反向步长、
  非连续目标和动态不合形失败均有严格 C17 执行回归。段上下界、步长、向量下标构造器及标量广播
  ...
  数组函数及嵌套变换结果现也会在块/单行 `IF`、短路 `ELSE IF`、算术 IF、`DO WHILE`、计数 DO
  起止步长、`SELECT CASE`、计算 `GOTO`、交替 `RETURN` 和 `STOP` 中通过统一临时量流程完整求值；
  标量选择值落地后立即清理拥有型结果，非拥有型指针结果不会被释放。不同数值 kind 的通用隐式
  提升、I/O 控制项和分配边界中的全部动态数组值，以及全部动态 CHARACTER/派生类型可选参数组合
  仍需纳入同一流程。
- [ ] 将参数、kind、字符长度、数组边界和初始化中的规格表达式全部纳入溢出安全的常量求值器，
  ...
  C17 组件常量，非法零除和复数零 `LOG` 会在生成前拒绝。仍需补齐其余规格 intrinsic 和所有
  允许出现位置。
  字符常量
...
- [x] 类型化表达式 IR 已删除生成代码、动态 extent、字符长度、数组临时量和有序实参物化等
  emitter 私有字段；这些状态现由编译上下文中的代码生成 overlay 独立拥有，并具有扩容、克隆、
  替换、子树释放和上下文清理测试。字符常量语义与字符 C17 生成也已拆分到各自职责模块。架构门禁
  禁止 AST/frontend/IR/semantic 重新依赖 lowering API，并禁止代码生成绕过状态清理直接释放临时
  表达式。
...
- [ ] 完整区分显式形状、假定大小、假定形状和延迟形状，统一标量、数组、字符、可分配及指针
  ...
  shape，以及 I/O 控制项、分配边界和后续完整 CFG 中的动态数组临时量生命周期，故本任务保持
  未关闭。分支、循环及选择语句的数组函数/变换结果已统一执行单次求值、标量值提交和所有权清理。
- [ ] 把字符长度、逐维 shape、实体属性、别名限制和嵌套过程签名纳入
  ...
  派生/多态类型和嵌套过程信息；
  显式过程实参会递归比较这些特征，函数结果同时检查 kind、shape、字符长度及所有权属性。字符
  长度按规格表达式的常量值比较，等价的 `len=2+2` 与 `len=4` 不会被误拒绝；kind、显式 extent、
  字符长度和结果所有权不一致均有独立负向回归。`PURE/ELEMENTAL` 过程特征、派生/多态参数和
  结果也已进入过程实参、项目定义及类型绑定覆盖检查；PURE 实参关联的标准单向放宽具有正向回归。
  `VALUE` 标量数据哑实参现通过隔离的过程入口副本实现：数值/逻辑/复数直接复制，CHARACTER 按
  声明长度截断和补空格，派生类型执行动态组件深复制并在所有退出路径逆序终结；`OPTIONAL VALUE`
  与 `VALUE,TARGET`、过程指针和内部过程均有严格 C17 执行回归。`TARGET/ASYNCHRONOUS/VOLATILE`
  会参与接口兼容和别名优化契约，VOLATILE 标量访问保留 C17 `volatile` 资格。尚需完成跨过程
  存储序列关联的别名集合、不能常量化的字符长度契约，以及 CHARACTER、派生类型和描述符对象的
  完整 VOLATILE 访问路径，故本任务保持未关闭。
...
- [ ] 补齐 `PROGRAM`、`MODULE`、`BLOCK DATA`、`SUBROUTINE`、`FUNCTION`、`ENTRY`、内部过程
  ...
  其可执行语句和非法嵌套会在生成前硬失败。单层内部过程现会从宿主符号表建立显式关联，并在
  typed IR 完成后按实际引用计算最小捕获集合；标量、固定形状数组、固定长度 CHARACTER 及宿主
  哑实参通过类型化隐藏参数传递，兄弟过程调用和递归调用会按宿主符号身份传播捕获；即使中间调用
  过程用局部同名声明遮蔽了宿主实体，也不会误传该局部变量。捕获也覆盖字符隐藏长度和局部声明中的
  规格表达式。宿主局部 `ALLOCATABLE`、数据 `POINTER`、deferred-length CHARACTER、动态上下界、
  extent、正负 stride 和分配来源现通过调用作用域描述符桥接；内部子程序、标量/字符/派生类型函数
  及可分配函数结果在重分配、释放、指针重关联、提前 `RETURN`、兄弟转发和递归调用后会双向同步
  宿主状态，带 `SAVE` 的局部动态实体也能跨宿主调用保持状态。含这些函数引用的二元表达式、用户
  过程实参和普通 intrinsic 实参会先按明确顺序物化，避免生成 C 中的未排序读写。严格 C17、
  ASan/UBSan 与原生 gfortran 差分已覆盖派生类型可分配数组及其动态组件。当前仍对宿主过程指针
  以及把带捕获的内部过程作为普通过程值逃逸的用法给出生成前硬错误；任意层嵌套、闭包感知过程值
  及完整多态动态实体捕获仍未实现。宿主已有显式接口的外部过程会继承完整签名；过程实参现会递归
  校验参数和结果的 type/kind/rank/shape、字符长度、动态属性、派生/多态身份、`PURE/ELEMENTAL`
  及嵌套过程签名。项目级显式接口与定义使用同一特征集合，包含
  `VALUE/TARGET/ASYNCHRONOUS/VOLATILE`；任意层宿主过程值仍需继续补齐。
  用户定义
...
- [ ] 完成显式/隐式接口、关键字与可选实参、过程实参、交替返回、字符隐藏长度和存储序列关联。
  旧式交替返回现已建立 canonical token → 表达式/语句 AST → typed IR → C17 emitter 的完整路径：
  `SUBROUTINE` 头保留普通与交替哑实参的完整位置布局，`CALL` 将 `*label` 绑定到对应交替槽，
  `RETURN selector` 生成整数选择器 ABI；普通、显式、泛型、模块、内部、未解析外部及过程哑实参调用
  均保留同一接口契约。语义阶段拒绝缺失、错位、畸形和未定义标签、非法选择器类型、接口布局冲突
  及 `ELEMENTAL` 组合；调用后的复制回写和临时量释放先于选择器分派，离开 `BLOCK` 的每个目标边
  复用作用域清理计划。显式过程实参的 kind、显式 shape、字符长度、结果 rank/shape 与
  `ALLOCATABLE/POINTER` 所有权契约现会在生成前校验，并允许语义等价的常量字符长度表达式。严格
  C17、ASan/UBSan 与原生 Fortran 差分已进入数值 CI。`VALUE` 已覆盖标量数据哑实参、可选参数、
  CHARACTER、含动态组件的派生类型、过程指针和内部过程；别名限制、字符隐藏长度的全部动态组合、
  存储序列关联和过程指针组件等其余接口组合仍未完成，故本任务保持未关闭。
...
- [ ] 完成命名及空白 `COMMON`、`EQUIVALENCE`、`DATA` 和 `SAVE` 的布局、初始化顺序、重叠与
  ...
  会在生成前硬失败。局部覆盖存储使用按职责拆分的类型化联合生成器，`DATA/SAVE` 生命周期仍传播
  到整组。`BLOCK DATA` 可将命名 `COMMON` 中的数值、逻辑、复数及
  ...
  `EQUIVALENCE` 类型化视图静态初始化命名 `COMMON`，包括从原块尾继续扩展的整数组；同一块中来自
  普通成员和关联别名的多个非重叠初始化区域会按绝对偏移合成为一个带静态断言的专用 C17 视图，
  初始化所有权仍按块检查，真正重叠的完整对象区域会在生成前硬失败。
  零大小数组和零长度 CHARACTER 现占用 0 个 Fortran 存储字节，不会改变后续 COMMON 成员偏移；
  生成端为其提供独立、带正确类型和对齐的最小 C 地址占位，跨程序单元布局与原生 Fortran 差分已
  覆盖。未对齐的数值、逻辑和复数关联实体现在使用原始字节存储及 `memcpy` 型访问器，不再构造未对齐
  C 指针；标量与整数组赋值、数组构造器、格式化/列表导向/非格式化 I/O、NAMELIST、显式形状和
  assumed-shape 过程实参、`INTENT(IN/OUT/INOUT)` 复制方向及只读函数实参均已接入，并由严格 C17、
  ASan/UBSan 和原生 Fortran 差分门禁覆盖。仍需完成部分数组元素之间的精细重叠一致性判定、未对齐
  非默认 kind CHARACTER、未对齐静态 `DATA/BLOCK DATA` 初始化及派生类型存储序列，因此本项保持
  未关闭。
...
- [x] 缺失模块现采用明确的三路解析策略：已实现的 intrinsic 模块、同一项目请求中的模块提供者，
  ...
  普通过程。公共 `F2cConfig` 提供有数量预算、Fortran 名称校验和大小写不敏感去重的
  `external_module_names`，CLI 通过可重复的 `--external-module NAME` 传入同一请求级契约。
  外部许可只声明提供者由请求之外供应，不会伪造模块成员接口；需要类型化导入的成员仍必须由
  项目内模块或后续外部接口清单提供。项目提供者晚于使用者输入、未知 intrinsic、非法/重复/超限
  外部名单和 CLI 两种参数形式均有独立正负向测试，架构门禁禁止恢复静默降级。本轮策略变更后，
  固定 Reference LAPACK 的 3,535 个源文件重新全部通过严格 C17 编译，DGESV、四个 INSTALL
  检查及 52,512 条 RFP 生成 C/原生 Fortran 差分全部通过。
...
- [ ] 完成 F90 `WHERE/ELSEWHERE`、所有合法 `DO` 形式、单行/块 IF 和旧式控制语句。命名
  ...
  IR 所有者；终止动作会在正确循环层级执行，循环后控制变量语义与原生 Fortran 差分一致。数组
  函数及嵌套变换结果已覆盖块/单行 IF、任意层短路 ELSE IF、算术 IF、每轮重算的 `DO WHILE`、
  只求值一次的计数 DO 参数、`SELECT CASE`、计算 `GOTO`、交替 `RETURN` 和 `STOP`；统一的标量
  提交边界会在控制转移前释放拥有型临时量。严格 C17、sanitizer 与原生 Fortran 副作用计数差分
  已进入默认 CTest 和数值验证 CI。
...
- [ ] 解释并消除固定审计基线中的未配对或超打印精度差异。当前统一审计记录为 5,807,798 条
  并集记录，其中 5,806,718 条双边配对，生成端和原生端各独有 540 条；850,762 条有限
...
- [ ] 完成 `TYPE/CLASS` 动态类型、`SELECT TYPE/SELECT RANK`、继承、抽象类型、类型绑定与延迟
  ...
  具体类型绑定覆盖现按标准校验 passed-object 的名称、位置、声明类型和标量/非指针/非可分配
  CLASS 契约；其余哑实参逐项比较名称、type/kind/rank/shape、字符长度、动态属性和嵌套过程，
  函数结果比较完整 shape、字符长度、派生/多态身份与所有权。`PURE/ELEMENTAL`、父 PUBLIC
  绑定不得降为 PRIVATE、DEFERRED 只能覆盖 DEFERRED、NON_OVERRIDABLE，以及重复/冲突绑定属性
  和绑定区默认 PRIVATE 均会在生成前诊断；动态覆盖执行和延迟绑定夹具继续通过严格 C17 与原生
  Fortran 差分。类型绑定可访问性的所有引用点、泛型覆盖组合、过程指针组件及完整多态过程值仍未
  完成，故本任务保持未关闭。
