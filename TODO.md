...
- [ ] 把字符长度、逐维 shape、`VALUE`、`OPTIONAL`、`INTENT`、别名限制和嵌套过程签名纳入
  ...
  `INTENT/OPTIONAL/ALLOCATABLE/POINTER/CONTIGUOUS`、描述符、派生/多态类型和嵌套过程信息；
  ...
  尚需补齐 `VALUE/TARGET/ASYNCHRONOUS/VOLATILE`、别名限制和不能常量化的字符长度约束，故本
  任务保持未关闭。
...
- [ ] 补齐 `PROGRAM`、`MODULE`、`BLOCK DATA`、`SUBROUTINE`、`FUNCTION`、`ENTRY`、内部过程
  ...
  及嵌套过程签名。项目级显式接口与定义使用同一特征集合；尚未建模的 `VALUE/TARGET` 等属性和
  任意层宿主过程值仍需继续补齐。
  用户定义
...
- [ ] 完成显式/隐式接口、关键字与可选实参、过程实参、交替返回、字符隐藏长度和存储序列关联。
  ...
  C17、ASan/UBSan 与原生 Fortran 差分已进入数值 CI。`VALUE`、别名限制、字符隐藏长度的全部动态
  组合、存储序列关联和过程指针组件等其余接口组合仍未完成，故本任务保持未关闭。
