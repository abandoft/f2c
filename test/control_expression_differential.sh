...
WORK=$ROOT/build/control-expression-differential
SOURCE=$ROOT/test/fixtures/control_expression.f90
...
"$FC" -std=legacy -O1 -Wall -Wextra -Werror -Wno-surprising \
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native control-expression behavior mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized control-expression output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "control-expression differential and sanitizer validation passed"
