...
WORK=$ROOT/build/common-equivalence-differential
SOURCE=$ROOT/test/fixtures/common_equivalence.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native COMMON/EQUIVALENCE output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized COMMON/EQUIVALENCE output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "COMMON/EQUIVALENCE differential and sanitizer validation passed"
