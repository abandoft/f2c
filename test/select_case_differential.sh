...
WORK=$ROOT/build/select-case-differential
SOURCE=$ROOT/test/fixtures/select_case.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native SELECT CASE output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi

echo "SELECT CASE differential passed"
