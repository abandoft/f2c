...
WORK=$ROOT/build/host-dynamic-association-differential
SOURCE=$ROOT/test/fixtures/host_dynamic_association.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native dynamic host-association behavior mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized dynamic host-association output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "dynamic host-association differential and sanitizer validation passed"
