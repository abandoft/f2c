...
WORK=$ROOT/build/equivalence-storage-differential
SOURCE=$ROOT/test/fixtures/equivalence_storage.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native EQUIVALENCE storage output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized EQUIVALENCE storage output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "EQUIVALENCE storage differential and sanitizer validation passed"
