...
WORK=$ROOT/build/alternate-return-differential
SOURCE=$ROOT/test/fixtures/alternate_return.f90
...
"$FC" -std=legacy -O2 -Wall -Wextra -Werror -J"$WORK" -I"$WORK" "$SOURCE" \
    -o "$WORK/native"
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native alternate-return behavior mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized alternate-return output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "alternate-return differential and sanitizer validation passed"
