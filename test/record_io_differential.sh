...
WORK=$ROOT/build/record-io-differential
SOURCE=$ROOT/test/fixtures/record_io.f90
...
if ! cmp -s "$WORK/generated/program.out" "$WORK/native/program.out"; then
    echo "generated/native record-I/O output mismatch" >&2
    diff -u "$WORK/native/program.out" "$WORK/generated/program.out" >&2 || true
    exit 1
fi

echo "record-I/O differential passed"
