...
WORK=$ROOT/build/block-finalization-differential
SOURCE=$ROOT/test/fixtures/block_finalization.f90
...
"$FC" -std=f2018 -pedantic -O2 -Wall -Wextra -Werror -J"$WORK" -I"$WORK" "$SOURCE" \
    -o "$WORK/native"
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native block-finalization behavior mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized block-finalization output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "block-finalization differential and sanitizer validation passed"
