...
static void test_program_and_control_flow(void) {
    ...
    expect_contains(result.code, "#define F2C_LOOP_UNROLL_OUTER",
                    "generated source defines a separate outer-loop optimization hint");
...
static void test_nested_loop_optimization_hints(void) {
    ...
    expect_contains(result.code, "F2C_LOOP_UNROLL_OUTER\n        for (;",
                    "the loop surrounding a vectorizable inner loop receives a smaller hint");
    expect(result.code == NULL ||
               strstr(result.code, "F2C_LOOP_UNROLL\n            for (;") == NULL,
           "the innermost loop remains available for vectorization after outer unroll-and-jam");
    f2c_result_free(&result);
}
