...
WORK=$ROOT/build/array-section-differential
SOURCE=$ROOT/test/fixtures/array_section_bounds_once.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native array-section output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi

echo "array-section operand evaluation differential passed"
