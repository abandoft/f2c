...
WORK=$ROOT/build/assigned-label-differential
SOURCE=$ROOT/test/fixtures/assigned_label_flow.f90
...
"$FC" -std=legacy -O2 -Wall -Wextra -Werror -Wno-surprising \
    -Wno-unused-label -Wno-aggressive-loop-optimizations -J"$WORK" -I"$WORK" "$SOURCE" \
    -o "$WORK/native"
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native assigned-label behavior mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized assigned-label output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "assigned-label differential and sanitizer validation passed"
