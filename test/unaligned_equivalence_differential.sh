...
WORK=$ROOT/build/unaligned-equivalence-differential
SOURCE=$ROOT/test/fixtures/unaligned_equivalence.f90
...
"$FC" -std=legacy -O2 -Wall -Wextra -Wno-align-commons "$SOURCE" -o "$WORK/native"
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native unaligned EQUIVALENCE output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized unaligned EQUIVALENCE output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "unaligned EQUIVALENCE differential and sanitizer validation passed"
