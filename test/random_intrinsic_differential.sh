...
WORK=$ROOT/build/random-intrinsic-differential
SOURCE=$ROOT/test/fixtures/random_intrinsics.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native random intrinsic property mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized random intrinsic output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "random intrinsic differential and sanitizer validation passed"
