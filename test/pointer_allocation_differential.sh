...
WORK=$ROOT/build/pointer-allocation-differential
SOURCE=$ROOT/test/fixtures/pointer_allocation.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native pointer-allocation behavior mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized pointer-allocation output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "pointer-allocation differential and sanitizer validation passed"
