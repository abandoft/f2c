...
WORK=$ROOT/build/legacy-control-differential
SOURCE=$ROOT/test/fixtures/legacy_control.f
...
"$F2C" --fixed-form "$SOURCE" -o "$WORK/generated.c"
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native legacy-control output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi

echo "legacy-control differential passed"
