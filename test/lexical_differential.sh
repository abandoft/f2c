...
WORK=$ROOT/build/lexical-differential
SOURCE=$ROOT/test/fixtures/lexical_literals.f90
...
"$CC" -std=c17 -O2 -Wall -Wextra -Wpedantic -Wconversion -Wshadow \
    ...
    -o "$WORK/generated"
"$FC" -std=legacy -O2 -Wall -Wextra -Wno-conversion "$SOURCE" -o "$WORK/native"

"$WORK/generated" > "$WORK/generated.out"
"$WORK/native" > "$WORK/native.out"

if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native lexical literal output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi

echo "lexical literal differential passed"
