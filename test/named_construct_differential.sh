...
WORK=$ROOT/build/named-construct-differential
SOURCE=$ROOT/test/fixtures/named_constructs.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native named-construct output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi

echo "named-construct differential passed"
