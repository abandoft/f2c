...
WORK=$ROOT/build/mathematical-conversion-intrinsic-differential
SOURCE=$ROOT/test/fixtures/mathematical_conversion_intrinsics.f90
...
if ! cmp -s "$WORK/generated.out" "$WORK/native.out"; then
    echo "generated/native mathematical and conversion intrinsic output mismatch" >&2
    diff -u "$WORK/native.out" "$WORK/generated.out" >&2 || true
    exit 1
fi
if ! cmp -s "$WORK/generated.out" "$WORK/generated-sanitized.out"; then
    echo "optimized/sanitized mathematical and conversion intrinsic output mismatch" >&2
    diff -u "$WORK/generated.out" "$WORK/generated-sanitized.out" >&2 || true
    exit 1
fi

echo "mathematical and conversion intrinsic differential validation passed"
